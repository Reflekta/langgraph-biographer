...
from react_agent.prompts import (
    ...
    QUESTION_CONTEXTUALIZATION_PROMPT,
...
class AnswerAnalysis(BaseModel):
    ...
    completeness_percentage: int = Field(
        description="How fully the answer addresses the question (0-100)", ge=0, le=100
    )
    quality_score: int = Field(
        description="Quality rating of the answer (1-10)", ge=1, le=10
    )
...
async def select_question_with_llm(
    state: State, priority: int, available_questions: List[dict]
) -> dict:
    ...
    questions_text = "\n".join(
        [f"ID: {q['id']} - {q['question']}" for q in available_questions]
    )
...
async def contextualize_question_with_llm(state: State, question: str) -> str:
    ...
    try:
        response = await llm.ainvoke([SystemMessage(content=contextualization_prompt)])
...
def check_interview_completion(questions: dict) -> bool:
    ...
    completed_questions = sum(
        1 for q in questions.values() if q["status"] == "complete"
    )
...
async def select_question_node(state: State) -> dict:
    ...
    need_new_question = not state.current_question_id or (
        state.current_question_id in questions_update
        and questions_update[state.current_question_id]["status"] == "complete"
    )

    # Also check if current question exists but is complete (redundant safety check)
    if (
        state.current_question_id
        and state.current_question_id in questions_update
        and questions_update[state.current_question_id]["status"] == "complete"
    ):
        ...
        for priority in PRIORITIES:
            ...
            if available_questions:
                ...
                if len(state.messages) > 2:
                    selected_question = await select_question_with_llm(
                        state, priority, available_questions
                    )
                else:
...
async def answer_analysis_node(state: State) -> dict:
    ...
    try:
        ...
        analysis_prompt = ANSWER_ANALYSIS_PROMPT.format(
            question=question_text, answer=latest_answer
        )
    ...
    if current_qid in questions_update:
        ...
        if analysis_result.status == "complete" or total_answers >= 2:
            ...
            if total_answers >= 2:
                questions_update[current_qid]["analysis"] += (
                    " (Marked complete after 2+ responses)"
                )
...
def make_welcome_message(configuration: Configuration) -> str:
    ...
    return (
        f"Hi {configuration.interviewee_name}, I'm William the Elder, your biographer. I'm here to help preserve {configuration.deceased_name}'s life story so you and your family can connect with them for generations to come.\n\n"
...
async def interview_agent(state: State) -> Dict[str, List[AIMessage]]:
    ...
    if len(state.messages) == 1:
        return {"messages": [AIMessage(content=make_welcome_message(configuration))]}
    ...
    if state.current_question_id and state.current_question_id in state.questions:
        ...
        if current_question_data["status"] == "in_progress":
            ...
            if len(state.messages) > 2:
                contextualized_question = await contextualize_question_with_llm(
                    state, question_text
                )
            else:
                contextualized_question = question_text

            return {"messages": [AIMessage(content=contextualized_question)]}
